...
"""
    measure_lattice_frohlich_state(rho, trace_observable, population_observables)

Measure site populations, trace, and TT-rank statistics for one HEOM-TT state.
"""
function measure_lattice_frohlich_state(rho, trace_observable, population_observables)
...
function _print_lattice_frohlich_progress(step, time_fs, measurement)
...
"""
    run_lattice_frohlich_dynamics(config, problem)

Propagate the configured initial site state with one Crank-Nicolson tAMEn
solve per saved time step.
"""
function run_lattice_frohlich_dynamics(config::LatticeFrohlichConfig, problem)
    ...
    measurement = measure_lattice_frohlich_state(
        rho,
    ...
    _print_lattice_frohlich_progress(0, times[1], measurement)
    ...
    for time_index in 2:save_count
        ...
        if step % config.progress_interval == 0 || step == step_count
            _print_lattice_frohlich_progress(step, times[time_index], measurement)
        end
    end
...
"""
    write_lattice_frohlich_population_csv(path, result)

Write all saved populations, trace values, and TT-rank diagnostics as CSV.
"""
function write_lattice_frohlich_population_csv(path, result)
