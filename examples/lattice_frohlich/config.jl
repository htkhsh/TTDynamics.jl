"""
    LatticeFrohlichConfig(; kwargs...)

Store the physical and numerical settings for a periodic lattice Fröhlich
HEOM-TT run. `brownian_damping_cm` is QFiND's Brownian damping parameter
`Γ_Q`; its poles decay at `Γ_Q / 2`.
"""
...
function LatticeFrohlichConfig(; site_count=5, site_energies_cm=zeros(site_count),
    ...
    return validate_lattice_frohlich_config(config)
end
...
"""
function validate_lattice_frohlich_config(config::LatticeFrohlichConfig)
