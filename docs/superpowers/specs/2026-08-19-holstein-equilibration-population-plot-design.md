...
`equilibrate.jl` imports CairoMakie and owns a small population plotting
function because plotting is executable presentation logic, not HEOM-TT model
logic. The plot consumes `result.times` and `result.populations` directly; it
does not read the CSV back from disk.
...
Use a small synthetic result to verify the PNG path, one line per site, axis
labels, and automatic invocation from the equilibrium save workflow. Add
regressions for no-overwrite protection, atomic PNG publication, and cleanup
after an injected plotting failure. Run the focused Holstein
current-correlation test file and the complete package test suite.
